...
if ($_GET["i"] == 1) {
	...
	if(substr_count($_SERVER['SERVER_SOFTWARE'], "lighttpd") > 0)
		Error("You're using lighttpd !", 'Please past this code into your lighttpd.conf<br /><br />url.rewrite-once = ( "^(.*)\.php$" => "veloce.php" )<br />url.rewrite-once = ( "^(.*\.)(css)$" => "veloce.php?minify&file=$1" )<br />server.error-handler-404 = "/veloce.php"<br />server.error-handler-500 = "/veloce.php"<br />server.error-handler-502 = "/veloce.php"<br />server.error-handler-400 = "/veloce.php"<br />server.error-handler-401 = "/veloce.php"<br />server.error-handler-403 = "/veloce.php"</br>server.dir-listing = "disable"');
